...
loop(Depth, MaxDepth, Iterations) ->
    ...
    io:format("~p trees of depth ~p check: ~p~n",
...
main(_) ->
    ...
    io:format("stretch tree of depth ~p check: ~p~n",
              [StretchDepth, check_tree(make_tree(0, StretchDepth))]),
    LongLivedTree = make_tree(0, MaxDepth),
    Iterations = round(math:pow(2, MaxDepth)),
    loop(MinDepth, StretchDepth, Iterations),
    io:format("long lived tree of depth ~p check: ~p~n",
              [MaxDepth, check_tree(LongLivedTree)]).
